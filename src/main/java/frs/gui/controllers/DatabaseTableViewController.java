        ...
        for (int i = 0; i < mFaults.length(); i++) {
            ...
            if (i >= faultData.size()) {
                faultData.add(new Fault_List(obj.getInt("fault_id"), obj.getInt("fault_no"),
                        obj.getString("fault_name"), obj.getString("fault_parameter"), String.valueOf(obj.getInt("fault_location")), obj.getString("symptom_desc"), obj.getJSONObject("reconf_command"),
                        obj.getString("occured_at")));
            }
        }
    }
}
